...
class TCMVE:
    def __init__(self, max_rounds: int = 5):
        ...
        with open(tlpo_path, "r", encoding="utf-8") as f:
    ...
    def run(self, query: str) -> Dict[str, Any]:
        ...
        for r in range(1, self.max_rounds + 1):
            ...
            gen_msg = (
                ...
                f"Ontology context: {self.ontology[:500]}..."
            )
...
"""
        # Run all 3 LLMs
        try:
            gen_resp = self.generator.invoke([{"role": "user", "content": eval_prompt}])
            gen_json = self._parse_json(gen_resp.content)
        except Exception as e:
            logger.error(f"Generator TLPO eval failed: {e}")
            gen_json = {}
        try:
            ver_resp = self.verifier.invoke([{"role": "user", "content": eval_prompt}])
            ver_json = self._parse_json(ver_resp.content)
        except Exception as e:
            logger.error(f"Verifier TLPO eval failed: {e}")
            ver_json = {}
        try:
            arb_resp = self.arbiter.invoke([{"role": "user", "content": eval_prompt}])
            arb_json = self._parse_json(arb_resp.content)
        except Exception as e:
            logger.error(f"Arbiter TLPO eval failed: {e}")
            arb_json = {}
